...
from app.services.cinemeta_resolver import resolve_cinemeta_episode
...
async def addon_content_sync(
    ...
    Supports:
    - mal:<mal_id>:<episode>
    - kitsu:<kitsu_id>:<episode>
    - Cinemeta IMDb IDs:
      tt1234567:<season>:<episode>

    Stremio calls this endpoint when the user watches content.
    """
    ...
    if content_id.startswith("tt"):
        try:
            # The resolver performs network requests, so run it in
            # a worker thread instead of blocking Quart's event loop.
            resolved = await asyncio.to_thread(
                resolve_cinemeta_episode,
                content_id,
            )
        except Exception as e:
            log_error(
                "CINEMETA_RESOLVER_ERROR",
                str(e),
                str(e),
                500,
            )
            ...
            return await respond_with(
                _create_sync_response(
                    status=UpdateStatus.FAIL,
                    message="Cinemeta resolver failed",
                ),
            )
        ...
        if not resolved:
            return await respond_with(
                _create_sync_response(
                    status=UpdateStatus.INVALID_ID,
                    message=f"Could not resolve Cinemeta ID: {content_id}",
                ),
                cache_max_age=config.CONTENT_SYNC_ON_INVALID_DURATION,
                stale_revalidate=config.CONTENT_SYNC_ON_INVALID_DURATION,
                stale_error=config.CONTENT_SYNC_ON_INVALID_DURATION,
        ...
        current_episode = resolved["episode"]

        print(
            f"Cinemeta resolved: "
            f"{content_id} -> "
            f"MAL {mal_id} episode {current_episode} "
            f"via {resolved.get('method', 'unknown')}"
        )

    # ============================================================
    # Existing MAL / Kitsu behavior
    # ============================================================
